...
const paymentsValidates = {
  paramsPayment: runSchema(Joi.object().keys({
    ...
    expiration: Joi.string().empty().required().pattern(/^([0-1]\d)\/(20\d{2})$/)
      .messages({
    ...
    cvv: Joi.string().empty().required().pattern(/^\d{3}/)
      .messages({
...
module.exports = paymentsValidates;

...
class PaymentsController {
    ...
    static async criaPayment(req, res) {
        ...
        if (!novoPayment.status || novoPayment.status === STATUS_CRIADO) {
            const novoPaymentCriado = await paymentsServices.criaRegistro(novoPayment, novoPayment.status = STATUS_CRIADO)
            const links = [
                {
                    "rel": "self",
                    "method": "GET",
        ...
        if(payment.status === STATUS_CRIADO){
            if(status === STATUS_CONFIRMADO) {
                try {
                    ...
                    const novaNotaFiscalCriada = await invoicesServices.criaRegistro({ descricao: descricao, paymentId: id}, {transaction: t})
                    await paymentsServices.atualizaRegistro({ status: STATUS_CONFIRMADO }, Number(id), {transaction: t})
                    return { id: id, status: STATUS_CONFIRMADO, notaFiscal: novaNotaFiscalCriada }
                    })
                    if(result.error){
                ...
                try {
                    ...
                    return res.status(200).json({ id: id, status: STATUS_CANCELADO })                
                } catch (error) {
                    return res.status(500).json(error.message)
                }
            } else {

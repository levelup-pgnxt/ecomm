...
O fator Processos implica na execução da aplicação em um ou mais processos sem estados. Quaisquer dados que necessitem de armazenamento são armazenados em serviços de apoio, no caso do nosso projeto são utilizdos mysql e mongodb.
...
Neste projeto utilizamos o Docker e é possível realizar a escalabilidade horizontal do mesmo.
...
Os eventos assíncronos são utilizados para que o retorno da comunicação ao cliente ocorra mesmo em casos de indisponibilidades momentaneas em alguns serviços, para isso, podem ser utilizados serviços de mensageria.

...
const validates = {
  ...
  paramsUser: runSchema(Joi.object().keys({
    nome: Joi.string().min(6).empty().required()
      .pattern(/^[A-Z].+/)
      .messages({
    ...
    cpf: Joi.string().empty().required().pattern(/^\d{11}$/)
      .messages({
    ...
    telefone: Joi.string().empty().required().pattern(/^\d{10,13}$/)
      .messages({
    ...
    endereco: Joi.object().keys({
      logradouro: Joi.string().min(6).empty().required()
        .pattern(/^[A-Z].+/)
        .messages({
      ...
      numero: Joi.string().min(1).empty().required()
        .pattern(/[a-z0-9//]+/i)
        .messages({
      ...
      complemento: Joi.string().min(3).empty().pattern(/^[A-Z].+/)
        .messages({
      ...
      bairro: Joi.string().min(3).empty().required()
        .pattern(/^[A-Z].+/)
        .messages({
      ...
      cidade: Joi.string().min(3).empty().required()
        .pattern(/^[A-Z].+/)
        .messages({
      ...
      uf: Joi.string().empty().required().pattern(/^[A-Z]{2}$/)
        .messages({
      ...
      cep: Joi.string().empty().required().pattern(/^\d{8}$/)
        .messages({
  ...
  somaCPF(cpf, dig) {
    let soma = 0;
    for (let i = 1; i <= dig - 2; i += 1) {
      soma += parseInt(cpf.substring(i - 1, i), 10) * (dig - i);
    }
  ...
  paramsCPF(cpf) {
    const validateCPF = (cpf) => {
      ...
      resto = this.somaCPF(cpf, 11);

      if ((resto === 10) || (resto === 11)) resto = 0;
      if (resto !== parseInt(cpf.substring(9, 10), 10)) return false;

      resto = this.somaCPF(cpf, 12);

...
const validates = {
  ...
  paramsProduct: runSchema(Joi.object().keys({
    ...
    descricao: Joi.string().min(limit).empty().required()
      .pattern(/^[A-Z].+/)
      .messages({
    ...
    slug: Joi.string().min(limit).empty().required()
      .pattern(/^[A-Za-z0-9-]+$/)
      .messages({
    ...
    precoUnitario: Joi.number().precision(2).greater(0).required()
      .messages({
    ...
    estoque: Joi.number().greater(0).less(10000).required()
      .messages({
  ...
  paramsUpdateProduct: runSchema(Joi.object().keys({
    nome: Joi.string().min(limit).empty().pattern(/^[A-Z].+/)
      .messages({
        'string.base': 'O campo "nome" deve ser do tipo texto!',
        'string.empty': 'O campo "nome" não deve ser vazio!',
        'string.min': `O campo "nome" deve ter no mínimo ${limit} caracteres!`,
    ...
    descricao: Joi.string().min(limit).empty().pattern(/^[A-Z].+/)
      .messages({
        'string.base': 'O campo "descrição" deve ser do tipo texto!',
        'string.empty': 'O campo "descrição" não deve ser vazio!',
        'string.min': `O campo "descrição" deve ter no mínimo ${limit} caracteres!`,
    ...
    slug: Joi.string().min(limit).empty().pattern(/^[A-Za-z0-9-]+$/)
      .messages({
        'string.base': 'O campo "slug" deve ser do tipo texto!',
        'string.empty': 'O campo "slug" não deve ser vazio!',
        'string.min': `O campo "slug" deve ter no mínimo ${limit} caracteres!`,
  ...
  valuesMaxMin: runSchema(Joi.object().keys({
    max: Joi.number().precision(2).greater(0).required()
      .messages({
    ...
    min: Joi.number().precision(2).greater(0).required()
      .messages({
  ...
  checkIsExistsCategory: async (nome) => {
  ...
  paramsID: (id) => {
    const isValideID = mongoose.Types.ObjectId.isValid(id);
    if (!isValideID) {
